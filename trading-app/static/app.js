...
async function fetchTradingConfig() {
  try {
    ...
    // Dynamically fetch and set strategy checkboxes
    try {
      const stratResp = await fetch('/api/all-strategies');
      const allStrategies = await stratResp.json();
      ...
      console.error('Failed to fetch all strategies:', e);
    }
